...
-   Add `babel-plugin-module-resolver` to fix aliases
-   Fix `getAccessToken` util.
...
-   Remove webpack aliases; use vscode ones.
...
-   Fallback to storage mock when indexed db cannot been opened

## 4.0.2 - 2020-04-21

### Updated

-   Set `checkTokenExpirationOnTabFocus` to `true` by default.
...
-   If tokens persistence is `NONE`, there was missing storage handler that would reset already stored data -> thus `reset` storage handler has been added.

## 4.0.0-beta.20 - 2019-12-05

### Fixed

-   If fetching auth. user fails during tokens retrieving, set the result to `false` and thus hide `loader` in `authorizable` HOC.
-   Clear storage state if tokens retrieval (including fetching user) fails.

### Updated

-   Map `authorizable` HOC to `entities.flowType`.
...
-   Upgrade dependencies.
-   Update babel config
    -   Use `@babel/preset-modules` for `process.env.BABEL_ENV === 'es'` -> package size decreased.
    -   Set `loose: true` -> package size decreased.
